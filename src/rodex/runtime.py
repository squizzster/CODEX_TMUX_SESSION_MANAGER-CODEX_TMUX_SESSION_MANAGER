...
from .process_contracts import AnalyticsRuntimeConfig, RuntimeServiceConfig
    ...
    def _refresh(self) -> None:
        for path in self._paths:
            try:
                ...
                raise RodexRuntimeError(f"could not refresh live runtime path {path}: {error}") from error
...
def _runtime_path_identity(path: Path) -> tuple[int, int, int, int]:
    try:
        ...
        raise RodexRuntimeError(f"could not inspect live runtime path {path}: {error}") from error
...
def _tmux_source_command_queue(arguments: Sequence[str]) -> str:
    ...
    return " ; ".join(" ".join(_tmux_source_byte_string(argument) for argument in command) for command in commands)
...
class RodexRuntimeLauncher:
    ...
    def __init__(
        ...
        *,
        ...
        daemon_client_factory: Callable[[Path, str], RodexDaemonClient] = RodexDaemonClient,
    ) -> None:
        ...
        self._user_process_environment = user_process_environment(os.environ if environment is None else environment)
    ...
    def _read_persisted_codex_session(
        ...
        try:
            ...
            raise RodexRuntimeError("thread/read returned an invalid Codex thread ID") from error
        if observed_id != codex_session_id:
            raise RodexRuntimeError(
                f"thread/read returned an unexpected Codex identity: requested {codex_session_id}, observed {observed_id}"
            )
    ...
    def start(
        ...
        *,
        ...
        _require_short_unix_socket_path(observer_control_socket_path(runtime.protocol_event_socket_path))
        ...
        try:
            capability, caller_environment_names = self._stage_tmux_session(
                runtime,
                resolved_workspace,
                session_environment,
            )
            analytics_config = AnalyticsRuntimeConfig(
            ...
            service_config = RuntimeServiceConfig(
                ...
                tmux_pane_target=capability.pane_target,
                runtime_id=runtime_id,
                codex_arguments=tuple(codex_arguments),
                analytics=analytics_config,
                user_environment=validated_user_environment_entries(session_environment),
            )
            daemon_client.reserve(operation_id, service_config)
            self._start_terminal_bridge(
                runtime,
                capability,
                operation_id,
            ...
            daemon_client.await_ready(operation_id, str(runtime_id), timeout_seconds=self._startup_timeout_seconds)
            ...
            if requested_codex_session_id is not None and codex_session_id != requested_codex_session_id:
                raise RodexRuntimeError(
            ...
            if service_config is not None:
                with suppress(Exception):
                    daemon_client.stop(operation_id, str(runtime_id))
    ...
    def publish_runtime_control(
        ...
        publication_action = " ; ".join(
            shlex.join(("set-option", "-t", target, option_name, value)) for option_name, value in options
        )
        self._tmux(
            runtime,
            "if-shell",
    ...
    def confirm_runtime_registration(
        ...
        *,
        ...
        if not isinstance(rodex_sessions_id, int) or isinstance(rodex_sessions_id, bool) or rodex_sessions_id <= 0:
    ...
    def discover_runtime_control(self, runtime: LiveTmuxSession) -> LiveRodexControl:
        ...
        if (
            ...
            or (tmux_session_id.startswith("$") and observed_tmux_session_id != tmux_session_id)
        ):
        ...
        try:
            ...
            raise RodexRuntimeError("live tmux session advertised an invalid Codex session ID") from error
        try:
            rodex_session_id = RodexSessionId.parse(rodex_session_id_text)
        except ValueError as error:
            raise RodexRuntimeError("live tmux session advertised an invalid Rodex session ID") from error
        try:
            rodex_registry_id = parse_rodex_registry_id(registry_id_text)
        except ValueError as error:
            raise RodexRuntimeError("live tmux session advertised an invalid Rodex registry ID") from error
        try:
            runtime_id = parse_rodex_runtime_id(runtime_id_text)
        except ValueError as error:
        ...
        if registration_state not in {
            ...
            raise RodexRuntimeError("live tmux registration state is invalid")
        ...
        if registration_state == RODEX_REGISTRATION_REGISTERED:
            try:
                ...
                raise RodexRuntimeError("registered live tmux capability is malformed") from error
    ...
    def rename(self, runtime: LiveTmuxSession, tmux_session_name: str) -> LiveTmuxSession:
        ...
        action = shlex.join(("rename-session", "-t", capability.session_target, session_name))
        self._tmux(
            runtime,
            "if-shell",
            "-t",
    ...
    def _list_registered_tmux_capabilities(
        ...
        for line in listed.stdout.splitlines():
            ...
            try:
                ...
                raise RodexRuntimeError("registered shared tmux capability is malformed") from error
        for projection in (
    ...
    def _resolve_registered_tmux_capability(
        ...
        if len(matches) != 1:
            ...
            raise RodexRuntimeError(f"registered runtime {runtime.runtime_id} was {detail} on shared tmux")
        return matches[0]
    ...
    def _install_shared_tmux_coordination(
        ...
        if installed_command != command:
            raise RodexRuntimeError("shared tmux coordinator belongs to a different Rodex installation")
        ...
        for event in ("attached", "detached", "session-changed"):
            ...
            if not _shell_commands_are_equivalent(verified_hook, command):
                raise RodexRuntimeError(f"shared tmux hook slot {hook_name} was not installed exactly")
    ...
    def _stage_tmux_session(
        self,
        runtime: LiveTmuxSession,
        workspace: Path,
        session_environment: Mapping[str, str],
    ) -> tuple[TmuxRuntimeCapability, frozenset[str]]:
        """Stage one exact inert pane and install its caller-owned environment."""
        if runtime.runtime_id is None:
        ...
        stale_global_environment_names = tuple(
            sorted(global_environment_names - caller_environment_names - TMUX_OWNED_CHILD_ENVIRONMENT_VARIABLES)
        )
        ...
        return capability, caller_environment_names

    def _start_terminal_bridge(
        self,
        runtime: LiveTmuxSession,
        capability: TmuxRuntimeCapability,
        operation_id: str,
        daemon_path: Path,
        caller_environment_names: frozenset[str],
        *,
        environment: Mapping[str, str],
    ) -> None:
        """Replace the staged pane with a one-shot exact TTY handoff to ``rodexd``."""
        bridge_command = (
            self._python_executable,
            "-I",
            "-m",
        ...
        if result.returncode != 0:
            raise RodexRuntimeError("terminal bridge could not replace the staged primary pane")
    ...
    def _read_tmux_global_environment_names(
        ...
        if result.returncode != 0:
            raise RodexRuntimeError("shared tmux global environment could not be read exactly")
        try:
    ...
    def capture_scrollback_snapshot(self, runtime: LiveTmuxSession) -> TmuxScrollbackSnapshot:
    ...
    def discover_current_tmux_pane_context(
        ...
        try:
            ...
            raise RodexRuntimeError("tmux returned an invalid attached-client count") from error
        ...
        *,
        ...
        while self._monotonic() < deadline:
            if not self._bootstrap_session_exists(runtime):
                ...
                if requested_codex_session_id is not None and _codex_reports_missing_session(
                    detail, requested_codex_session_id
                ):
                    raise RodexCodexSessionNotFoundError(
                        f"Codex has no saved session for exact identity {requested_codex_session_id}"
                    )
                ...
                try:
                    ...
                    if len(loaded) == 1:
                        try:
                            ...
                            raise RodexRuntimeError(
                                f"app-server returned an invalid Codex session ID: {loaded[0]!r}"
                            ) from error
                    if len(loaded) > 1:
                        raise RodexRuntimeError("private app-server loaded more than one Codex thread")
    ...
    def _list_loaded_codex_threads(self, socket_path: Path, peer_identity: RuntimePeerIdentity) -> list[str]:
        with verified_runtime_connection(
            ...
            websocket.send(json.dumps(CODEX_APP_SERVER.initialize_request(0, RODEX_RUNTIME_APP_SERVER_CLIENT)))
            initialized = _receive_response(websocket, 0)
            CODEX_APP_SERVER.require_minimum_version(initialized)
            websocket.send(json.dumps(CODEX_APP_SERVER.initialized_notification()))
            websocket.send(json.dumps(CODEX_APP_SERVER.request(1, CODEX_APP_SERVER.thread_loaded_list_method, {})))
    ...
    def _tmux(
        ...
        *arguments: str,
        ...
        deadline = RODEX_TMUX_COMMAND_TIMEOUT_SECONDS if timeout_seconds is None else timeout_seconds
        ...
        if result.timed_out:
            raise RodexRuntimeError(f"tmux command timed out after {deadline:g}s: {arguments[0]}")
    ...
    def _install_owned_root_key_binding(
        ...
        *,
        ...
        if owned_command and not _shell_commands_are_equivalent(owned_command, command):
            raise RodexRuntimeError(f"{contract_name} command is owned by a different Rodex installation")
        if existing_command and not _shell_commands_are_equivalent(
            existing_command,
            command,
        ):
            if owned_command:
                raise RodexRuntimeError(f"{contract_name} binding changed after Rodex claimed it")
            raise RodexRuntimeError(f"cannot install the {contract_name}: root {key} already has a non-Rodex binding")
        self._server_capability_tmux(
            runtime,
            capability.tmux_server_id,
            "set-option",
        ...
        if installed_command != command:
            raise RodexRuntimeError(f"{contract_name} binding belongs to a different Rodex installation")
        if not existing_command:
            ...
            if claimed_binding and not _shell_commands_are_equivalent(
                ...
                raise RodexRuntimeError(f"{contract_name} binding changed while Rodex claimed it")
            self._server_capability_tmux(
                runtime,
                capability.tmux_server_id,
                "bind-key",
...
def run_runtime_service(
    ...
    *,
    ...
    on_analytics_activated: Callable[[AnalyticsRuntimeConfig], None] = lambda _config: None,
    ...
    try:
        return _run_runtime_service(
            ...
            on_analytics_activated=on_analytics_activated,
...
def _run_runtime_service(
    ...
    *,
    ...
    on_analytics_activated: Callable[[AnalyticsRuntimeConfig], None],
    ...
    unexpected_terminal_names = set(terminal_environment) - TMUX_OWNED_CHILD_ENVIRONMENT_VARIABLES
    if unexpected_terminal_names:
        raise RodexRuntimeError("terminal bridge environment contains caller-owned names")
    user_environment.update(terminal_environment)
    ...
    pending_analytics_config: AnalyticsRuntimeConfig | None = config.analytics
    tmux_pane_target = config.tmux_pane_target
    tmux_runtime_capability = _resolve_runtime_service_tmux_capability(
        tmux_binary,
    ...
    registered_interaction_context: AnalyticsRuntimeConfig | None = None
    ...
    try:
        ...
        with _open_private_runtime_log(app_server_log_path) as log:
            ...
            native_tui_process = BoundProcessOwner()
            protocol_proxy = CodexProtocolProxy(
                ...
                native_tui_process=native_tui_process,
            ...
            while True:
                ...
                terminal_gateway = TerminalSessionGateway(
                    ...
                    **tui_options,
                    ...
                    on_process_started=lambda process: on_process_started("native-tui", process),
                ...
                while True:
                    if pending_analytics_config is not None:
                        activated_analytics = _registered_analytics_runtime_config(
                            pending_analytics_config,
                            tmux_binary,
                            tmux_server_socket_path,
                            tmux_pane_target,
                        )
                        if activated_analytics is not None:
                            ...
                            # Persistent statistics remain fail-open and must not
                            # interrupt the daemon's interactive runtime pipeline.
                            try:
                                on_analytics_activated(activated_analytics)
                            except Exception:
                                pass
                            else:
                                pending_analytics_config = None
                        elif time.monotonic() >= registration_deadline:
                            raise RodexRuntimeError("runtime registration was not confirmed before its deadline")
...
def _resolve_runtime_service_tmux_capability(
    tmux_binary: str,
    tmux_server_socket_path: Path,
    ...
    """Bind one daemon runtime to the exact tmux incarnation that spawned its bridge."""
    if not tmux_pane_target:
        raise RodexRuntimeError("runtime service has no exact tmux pane identity")
    ...
    if (
        ...
        raise RodexRuntimeError("runtime service tmux incarnation could not be verified")
    try:
        ...
        raise RodexRuntimeError("runtime service tmux capability is malformed") from error


def admit_runtime_terminal_fd(
    config: RuntimeServiceConfig,
    terminal_fd: int,
    *,
    peer_pid: int,
) -> tuple[TmuxRuntimeCapability, dict[str, str]]:
    """Bind one passed pane TTY to its exact tmux/runtime/process capability."""
    capability = _resolve_runtime_service_tmux_capability(
        config.tmux_binary,
    ...
    observed = SyncTmuxExecutor(
        config.tmux_binary,
        config.tmux_server_socket_path,
        runner=subprocess.run,
    ).run(primary_pane_capability_read_arguments(capability, "#{pane_pid}\t#{pane_tty}"))
    fields = observed.stdout.rstrip("\n").split("\t")
    if observed.returncode != 0 or len(fields) != 2 or not fields[0].isdigit() or int(fields[0]) != peer_pid:
        raise RodexRuntimeError("terminal bridge process does not own the exact primary pane")
    terminal_path = Path(fields[1])
    try:
        descriptor_state = os.fstat(terminal_fd)
        path_state = terminal_path.stat()
        descriptor_path = Path(os.ttyname(terminal_fd))
    except (OSError, ValueError) as error:
        raise RodexRuntimeError("terminal bridge did not pass a live pane TTY") from error
    if (
        not stat_module.S_ISCHR(descriptor_state.st_mode)
        or descriptor_state.st_uid != os.getuid()
        or descriptor_path != terminal_path
        or (descriptor_state.st_dev, descriptor_state.st_ino, descriptor_state.st_uid)
        != (path_state.st_dev, path_state.st_ino, path_state.st_uid)
    ):
        raise RodexRuntimeError("terminal bridge TTY does not match the exact primary pane")
    try:
        inherited_environment = _read_process_environment(peer_pid)
        pane_identity = _tmux_pane_id_from_environment(inherited_environment.get("TMUX_PANE"))
        tmux_socket_path = _tmux_socket_path_from_environment(inherited_environment.get("TMUX"))
    except (OSError, UnicodeError, RodexRuntimeError, ValueError) as error:
        raise RodexRuntimeError("terminal bridge environment could not be verified") from error
    if pane_identity != config.tmux_pane_target or tmux_socket_path != config.tmux_server_socket_path:
        raise RodexRuntimeError("terminal bridge environment does not match the exact primary pane")
    terminal_environment = {
        name: inherited_environment[name]
        for name in TMUX_OWNED_CHILD_ENVIRONMENT_VARIABLES
        if name in inherited_environment
    }
    if not terminal_environment.get("TERM"):
        raise RodexRuntimeError("terminal bridge environment has no terminal type")
    return capability, terminal_environment


def _read_process_environment(pid: int) -> dict[str, str]:
    """Read one live bridge's exec-time environment without accepting duplicates."""
    if type(pid) is not int or pid <= 0:
        raise ValueError("process ID must be positive")
    content = Path(f"/proc/{pid}/environ").read_bytes()
    ...
    for entry in content.split(b"\0"):
        ...
        if not separator:
            raise ValueError("process environment entry has no value")
        name = os.fsdecode(name_bytes)
        value = os.fsdecode(value_bytes)
        if not name or name in environment:
            raise ValueError("process environment contains an invalid or duplicate name")
...
def _registered_analytics_runtime_config(
    config: AnalyticsRuntimeConfig,
    tmux_binary: str,
    tmux_server_socket_path: Path,
    tmux_pane_target: str,
) -> AnalyticsRuntimeConfig | None:
    ...
    for option_name, expected_value in expected.items():
        if options.get(option_name) != expected_value:
            raise RodexRuntimeError(f"registered analytics identity disagrees at {option_name}")
    try:
        ...
        raise RodexRuntimeError("registered analytics identity is missing or invalid") from error
...
def _is_missing_persisted_codex_session(error: object, codex_session_id: CodexSessionId) -> bool:
    return (
...
def _prepare_runtime_root(root: Path) -> Path:
    ...
    if before.st_uid != os.getuid():
        raise RodexRuntimeError(f"runtime directory is not owned by uid {os.getuid()}: {root}")
...
def _require_secure_runtime_parent(parent: Path) -> None:
    ...
    if state.st_uid == 0 and state.st_mode & stat_module.S_ISVTX:
        return
    raise RodexRuntimeError(f"runtime parent is not private or root-owned sticky storage: {parent}")
...
def _codex_reports_missing_session(detail: str, requested_codex_session_id: CodexSessionId) -> bool:
...
def _codex_reports_active_writer(detail: str, requested_codex_session_id: CodexSessionId) -> bool:
...
def _active_writer_handoff_can_retry(
    ...
    *,
    ...
    return now < deadline and _codex_reports_active_writer(detail, requested_codex_session_id)
