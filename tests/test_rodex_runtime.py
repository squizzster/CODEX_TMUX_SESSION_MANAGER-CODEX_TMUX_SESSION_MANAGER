...
def test_runtime_service_releases_transferred_terminal_on_early_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    terminal_fd, peer_fd = os.pipe()

    def fail_before_runtime_setup(*_args: object, **_kwargs: object) -> int:
        raise RodexRuntimeError("invalid runtime capability")

    monkeypatch.setattr(runtime_module, "_run_runtime_service", fail_before_runtime_setup)
    try:
        with pytest.raises(RodexRuntimeError, match="invalid runtime capability"):
            run_runtime_service(
                cast(RuntimeServiceConfig, object()),
                terminal_fd=terminal_fd,
                terminal_environment={},
                stop=Event(),
            )
        with pytest.raises(OSError):
            os.fstat(terminal_fd)
    finally:
        os.close(peer_fd)
    ...
    def __call__(self, command: list[str], **options: object) -> subprocess.CompletedProcess[str]:
        ...
        if command[3:] == ["source-file", "-"]:
            return subprocess.CompletedProcess(
                ...
                1,
                stdout="",
                stderr="injected environment install failure",
            )
        return result
...
class RecordingDaemonClient:
    ...
    def await_ready(self, operation_id: str, runtime_id: str, *, timeout_seconds: float) -> None:
        self.ready.append((operation_id, runtime_id, timeout_seconds))
...
class RecordingDaemonFactory:
    def __init__(self) -> None:
        self.clients: list[RecordingDaemonClient] = []

    def __call__(self, runtime_root: Path, _python_executable: str) -> RecordingDaemonClient:
        client = RecordingDaemonClient(runtime_root)
        self.clients.append(client)
        return client


def _stage_test_tmux_command(
    launcher: RodexRuntimeLauncher,
    runtime: LiveTmuxSession,
    workspace: Path,
    command: tuple[str, ...],
) -> TmuxRuntimeCapability:
    """Exercise staging, then replace its inert pane with a test-native command."""
    environment = launcher._user_process_environment.copy()
    environment["PWD"] = str(workspace)
    capability, caller_environment_names = launcher._stage_tmux_session(runtime, workspace, environment)
    launcher._tmux(
        runtime,
        "respawn-pane",
        "-k",
...
def test_start_directly_hosts_codex_in_tmux_and_returns_its_session_id(
    ...
    respawn = shlex.split(runner.calls[respawn_index][-2])
    host_start = respawn.index("/venv/bin/python")
    host_command = respawn[host_start:]
    ...
    assert "--environment-name=USER_SETTING" in host_command
    scrubber_boundary = host_command.index("--")
    real_host_command = host_command[scrubber_boundary + 1 :]
    assert real_host_command[:4] == [
        "/venv/bin/python",
        "-I",
        "-m",
    ...
    reserved = daemon_factory.clients[0].reservations[0][1]
    assert reserved.codex_arguments == ("--model", "example")
    assert reserved.analytics.rodex_database_path == tmp_path / "rodex.sqlite3"
    assert reserved.analytics.codex_sessions_root == runtime_module.default_codex_sessions_root()
    assert reserved.user_environment == (("PATH", "/usr/bin"), ("PWD", str(tmp_path)), ("USER_SETTING", "preserved"))
    assert runner.options[new_session_index]["env"] == {
        "PATH": "/usr/bin",
        "PWD": str(tmp_path),
        "USER_SETTING": "preserved",
    }
    assert runner.options[respawn_index]["env"] == {
        "PATH": "/usr/bin",
        "PWD": str(tmp_path),
        "USER_SETTING": "preserved",
    }
...
def test_startup_environment_failure_never_starts_host_and_removes_placeholder(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("RODEX_RUNTIME_DIR", str(tmp_path))
    ...
    runner = FailingEnvironmentInstallRunner(tmp_path)
    launcher = RodexRuntimeLauncher(
        "/usr/bin/codex",
        "/usr/bin/tmux",
        runner=runner,
...
def test_startup_cleanup_failure_is_not_allowed_to_replace_the_causal_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("RODEX_RUNTIME_DIR", str(tmp_path))
    ...
    def fail_cleanup(*_arguments: object, **_options: object) -> None:
        raise RodexRuntimeError("cleanup capability changed")

    monkeypatch.setattr(launcher, "_stage_tmux_session", fail_start)
...
def test_tmux_global_environment_parser_ignores_multiline_assignment_lookalikes() -> None:
    shell_environment = (
        'NORMAL="value"; export NORMAL;\n'
        'MULTILINE="first\nFAKE_ASSIGNMENT=not-a-name\nlast"; export MULTILINE;\n'
        "unset REMOVED;\n"
    )

    assert runtime_module._parse_tmux_shell_environment_names(shell_environment) == {
        "MULTILINE",
        "NORMAL",
    }


def test_tmux_global_environment_parser_preserves_non_shell_identifier_names() -> None:
    shell_environment = (
        '#HASH="value"; export #HASH;\n'
        'HAS SPACE="value"; export HAS SPACE;\n'
        'HAS;SEMICOLON="value"; export HAS;SEMICOLON;\n'
        "HAS'QUOTE=\"value\"; export HAS'QUOTE;\n"
    )
...
def test_tmux_global_environment_read_fails_closed(
    tmp_path: Path,
) -> None:
    def runner(command: list[str], **_options: object) -> subprocess.CompletedProcess[str]:
        assert "show-environment" in command[-2]
    ...
    runtime = LiveTmuxSession(tmp_path / "tmux.sock", "session", runtime_id=RUNTIME_ID)

    with pytest.raises(RodexRuntimeError, match="could not be read exactly"):
        launcher._read_tmux_global_environment_names(
            runtime,
            "0123456789abcdef0123456789abcdef",
        )


def test_start_passes_the_exact_leading_zero_session_id_to_the_daemon(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    codex_session_id = "01a00654-f2bc-7a30-834a-a5f886a65f82"
    ...
    reserved = daemon_factory.clients[0].reservations[0][1]
    assert reserved.analytics.rodex_session_id == rodex_session_id
    assert reserved.analytics.rodex_database_path == tmp_path / "rodex-v3.sqlite3"
...
def test_fresh_runtime_service_survives_initialization(
    tmp_path: Path,
) -> None:
...
activated = AnalyticsRuntimeConfig(
...
raise SystemExit(
    runtime.run_runtime_service(
        config,
        terminal_fd=0,
        terminal_environment={"TERM": "xterm-256color", "TMUX": f"{tmux_socket},1,0", "TMUX_PANE": "%1"},
        stop=Event(),
        on_analytics_activated=activate_analytics,
    )
)
"""
    environment = os.environ.copy()
    ...
    try:
        ...
        while not ready.exists() and process.poll() is None:
            if time.monotonic() >= deadline:
                break
            time.sleep(0.02)
        startup_error = "runtime service startup timed out"
        if process.poll() is not None:
            _debug_stdout, debug_stderr = process.communicate(timeout=3)
            startup_error = f"runtime service exited {process.returncode}: {debug_stderr}"
...
def test_real_tmux_ctrl_d_survives_hostile_unattached_user_options(
    ...
    try:
        ...
        _stage_test_tmux_command(launcher, runtime, tmp_path, ("/bin/sleep", "30"))
...
def test_rodex_does_not_override_user_mouse_preferences(
    ...
    launcher._stage_tmux_session(runtime, tmp_path, {"PATH": "/usr/bin", "PWD": str(tmp_path)})
...
def test_real_tmux_session_replaces_stale_rodex_environment_with_caller_state(
    ...
    stale_environment.update(
        {
            ...
            "VIRTUAL_ENV_PROMPT": "(rodex)",
            "UV_RUN_RECURSION_DEPTH": "1",
            "USER_SETTING": "stale-server-value",
            "STALE_ONLY": "must-not-survive",
            "STALE_MULTILINE": "first\nFAKE_ASSIGNMENT=still-a-value\nlast",
            "BASH_ENV": "/stale/bash-env",
            "ENV": "/stale/posix-env",
            "LD_LIBRARY_PATH": "/stale/loader-path",
            "PYTHONHOME": "/stale/python-home",
            "PYTHONPATH": "/stale/python-path",
            "STALE SPACE": "must-not-survive",
        }
    )
    subprocess.run(
        [
    ...
    caller_environment = {
        "HOME": str(tmp_path),
        "PATH": "/usr/bin",
        "SHELL": "/caller/shell-must-not-win",
        "TERM": "xterm-256color",
        "TERM_PROGRAM": "caller-terminal-must-not-win",
        "SHLVL": "27",
        "_": "caller-owned-last-command",
        "UV_RUN_RECURSION_DEPTH": "",
        "USER_SETTING": "current-caller-value",
        "-OPTION_SHAPED": "option-name-value",
        "CALLER SPACE": "caller-space-value",
        "CALLER_ONLY": 'line one\n\'"quotes"; \\$HOME = #{literal} ✓\tend',
        "NON_UTF8": os.fsdecode(b"caller-\xff-value"),
    }
    ...
    primary_probe_path = tmp_path / "managed-primary-environment.json"
    later_probe_path = tmp_path / "managed-later-pane-environment.json"
    probe_source = (
        "import json, os, sys, time; from pathlib import Path; "
        "names = ('PATH', 'VIRTUAL_ENV', 'VIRTUAL_ENV_PROMPT', "
        "'UV_RUN_RECURSION_DEPTH', 'USER_SETTING', '-OPTION_SHAPED', "
        "'CALLER SPACE', 'CALLER_ONLY', 'NON_UTF8', "
        "'STALE_ONLY', 'STALE_MULTILINE', 'BASH_ENV', 'ENV', "
        "'LD_LIBRARY_PATH', 'PYTHONHOME', 'PYTHONPATH', 'STALE SPACE', "
        "'LATE_GLOBAL', "
        "'PWD', 'SHELL', 'SHLVL', '_', "
        "'TERM', 'TERM_PROGRAM', 'TERM_PROGRAM_VERSION', 'TMUX', 'TMUX_PANE'); "
        "observed = {name: os.environ.get(name) for name in names}; "
        "observed['EMPTY_ARG'] = sys.argv[2]; "
        "Path(sys.argv[1]).write_text(json.dumps(observed)); time.sleep(10)"
    )
    launcher = RodexRuntimeLauncher(
        "codex",
        tmux_binary,
    ...
    try:
        _stage_test_tmux_command(
            ...
            runtime,
            tmp_path,
            (sys.executable, "-c", probe_source, str(primary_probe_path), ""),
        )
        subprocess.run(
            [
                tmux_binary,
                ...
                "-g",
                "LATE_GLOBAL",
                "must-not-cross-the-exec-boundary",
            ],
            check=True,
        )
        ...
        for probe_path in (primary_probe_path, later_probe_path):
            ...
            assert observed["EMPTY_ARG"] == ""
            assert observed["CALLER_ONLY"] == caller_environment["CALLER_ONLY"]
            assert observed["NON_UTF8"] == caller_environment["NON_UTF8"]
            assert observed["STALE_ONLY"] is None
            assert observed["STALE_MULTILINE"] is None
            assert observed["BASH_ENV"] is None
            assert observed["ENV"] is None
            assert observed["LD_LIBRARY_PATH"] is None
            assert observed["PYTHONHOME"] is None
            assert observed["PYTHONPATH"] is None
            assert observed["STALE SPACE"] is None
            assert observed["LATE_GLOBAL"] is None
            assert observed["SHLVL"] == caller_environment["SHLVL"]
            assert observed["_"] == caller_environment["_"]
            assert observed["SHELL"] != caller_environment["SHELL"]
            assert observed["TERM"].startswith("screen")
            assert observed["TERM_PROGRAM"] == "tmux"
            assert observed["TERM_PROGRAM_VERSION"]
            assert str(socket_path) in observed["TMUX"]
            assert observed["TMUX_PANE"].startswith("%")
        primary_observed = json.loads(primary_probe_path.read_text(encoding="utf-8"))
        later_observed = json.loads(later_probe_path.read_text(encoding="utf-8"))
...
def test_real_concurrent_first_sessions_keep_distinct_caller_environments(
    tmp_path: Path,
) -> None:
    tmux_binary = shutil.which("tmux")
    if tmux_binary is None:
    ...
    probes = {
        "first": tmp_path / "first-environment.json",
        "second": tmp_path / "second-environment.json",
    }
    probe_source = (
        "import json, os, sys, time; from pathlib import Path; "
        "names = ('PATH', 'FIRST_ONLY', 'SECOND_ONLY', 'PWD', 'TMUX_PANE'); "
    ...
    def launch(
        ...
        try:
            ...
            _stage_test_tmux_command(
                ...
                tmp_path,
                (sys.executable, "-c", probe_source, str(probes[session_name])),
            )
        except BaseException as error:
    ...
    threads = (
        ...
        Thread(
            ...
            args=(
                "second",
                RodexRuntimeId.parse("2222222222222222"),
                {
                    "HOME": str(tmp_path),
                    "PATH": "/second/bin:/usr/bin",
    ...
    try:
        ...
        second = json.loads(probes["second"].read_text(encoding="utf-8"))
        assert first == {
            "PATH": "/first/bin:/usr/bin",
            "FIRST_ONLY": "first-value",
            "SECOND_ONLY": None,
            "PWD": str(tmp_path),
            "TMUX_PANE": first["TMUX_PANE"],
        }
        assert first["TMUX_PANE"].startswith("%")
        assert second == {
            "PATH": "/second/bin:/usr/bin",
            "FIRST_ONLY": None,
            "SECOND_ONLY": "second-value",
...
def test_real_tmux_survives_rename_and_status_configuration(tmp_path: Path) -> None:
    ...
    # display-message width formatting.
    subprocess.run(
        [
            tmux_binary,
            "-S",
    ...
    for dummy_name in ("pane-id-one", "pane-id-two", "pane-id-three"):
        subprocess.run(
            [
                tmux_binary,
                "-S",
...
def test_registered_analytics_config_binds_the_committed_identity_once(
    ...
    registry_id = RodexRegistryId.parse("06179a3581264d53")
    event_socket = tmp_path / f"events-{RUNTIME_ID}.sock"
    pending = AnalyticsRuntimeConfig(
        tmux_server_id="0123456789abcdef0123456789abcdef",
        rodex_database_path=tmp_path / "rodex.sqlite3",
    ...
    activated = runtime_module._registered_analytics_runtime_config(
        pending,
        "/usr/bin/tmux",
        tmp_path / "tmux.sock",
        "%4",
...
def test_runtime_service_skips_updater_and_connects_tui_through_protocol_proxy(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    def activate_analytics(config: AnalyticsRuntimeConfig) -> None:
        assert config.rodex_database_path == tmp_path / "rodex.sqlite3"
        proxy_lifecycle.append("analytics-delegated")
    ...
    if bootstrap_virtualenv:
        ...
        monkeypatch.setenv("VIRTUAL_ENV_PROMPT", "")
        monkeypatch.setenv("UV_RUN_RECURSION_DEPTH", "")
    ...
    assert (
        run_runtime_service(
            RuntimeServiceConfig(
                ...
                user_environment=validated_user_environment_entries(runtime_module.user_process_environment(os.environ)),
            ),
    ...
    assert proxy_lifecycle == [
        ...
        "start",
        "keepalive-start",
        *(["observer-activate", "analytics-delegated"] if analytics_start_fails else []),
        "keepalive-close",
        "close",
        "observer-close",
        "event-close",
    ...
    for process_environment in spawned_environments:
        ...
        if bootstrap_virtualenv:
            ...
            assert process_environment["VIRTUAL_ENV"] == "/project/.venv"
            assert process_environment["VIRTUAL_ENV_PROMPT"] == ""
            assert process_environment["UV_RUN_RECURSION_DEPTH"] == ""
    if captures_stderr:
...
def test_runtime_service_retries_exact_resume_during_active_writer_handoff(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    gateways = _mock_terminal_gateway(monkeypatch)
...
def test_runtime_service_terminates_the_tui_when_runtime_keepalive_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    gateways = _mock_terminal_gateway(monkeypatch)
    lifecycle: list[str] = []
