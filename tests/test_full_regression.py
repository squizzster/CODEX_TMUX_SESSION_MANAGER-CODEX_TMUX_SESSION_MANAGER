...
def test_full_rodex_database_regression(tmp_path: Path) -> None:
    ...
    first = create_a_rodex_session(database, codex_session_id=uuid.UUID(int=1), user_identity=user)
    second = create_a_rodex_session(database, codex_session_id=uuid.UUID(int=2), user_identity=user)

    assert [first.rodex_sessions_id, second.rodex_sessions_id] == [1, 2]
    assert first.rodex_session_id != second.rodex_session_id
    assert lookup_rodex_sessions_id_from_a_rodex_session_id(first.rodex_session_id, database) == first.rodex_sessions_id
    assert lookup_rodex_session_id_from_a_rodex_sessions_id(second.rodex_sessions_id, database) == second.rodex_session_id
    ...
    with sqlite3.connect(database) as connection:
        assert connection.execute("SELECT id, uid, gid, user_name FROM rodex_sessions_users").fetchall() == [
            (1, 1009, 1010, "dna")
        ]
        assert connection.execute(
            "SELECT rodex_sessions_id, rodex_sessions_users_id FROM rodex_sessions_log ORDER BY id"
        ).fetchall() == [(1, 1), (2, 1)]
        assert connection.execute("SELECT cool_names_id FROM rodex_sessions ORDER BY id").fetchall() == [(1,), (2,)]
        assert connection.execute("SELECT name, seq FROM sqlite_sequence ORDER BY name").fetchall() == [
            ("codex_threads", 2),
            ("cool_names", 2),
            ("rodex_registries", 1),
