...
def test_checked_in_contract_is_generated_from_the_minimum_supported_cli(
    tmp_path: Path,
) -> None:
    """Current evidence is the schema generated by the minimum supported build."""
    ...
    if version != f"codex-cli {CODEX_APP_SERVER.minimum_version}":
        pytest.skip(f"installed CLI is not the fixture baseline: {version}")
...
def test_live_initialize_metadata_rejects_missing_version() -> None:
    with pytest.raises(RodexAppServerVersionError, match="no recognized"):
        CODEX_APP_SERVER.require_minimum_version({})
